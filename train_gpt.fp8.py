...
def block_attn_res(
    ...
    Optimized for FP8 computation on H100.
    """
...
class CastedLinear(nn.Linear):
    """Linear layer that keeps weights in fp32 but casts to computation dtype."""
    def forward(self, x: Tensor) -> Tensor:
        bias = self.bias.to(x.dtype) if self.bias is not None else None
        return F.linear(x, self.weight.to(x.dtype), bias)
...
def restore_low_dim_params_to_fp32(module: nn.Module) -> None:
    """Keep small/control parameters in fp32 even when model runs in lower precision."""
    with torch.no_grad():
...
class CausalSelfAttention(nn.Module):
    """Multi-head self-attention with GQA support, optimized for FP8."""
    def __init__(
        self,
        dim: int,
        num_heads: int,
...
class MLP(nn.Module):
    """ReLU^2 MLP optimized for FP8 computation."""
...
class Block(nn.Module):
    """Transformer block with Block Attention Residuals, FP8-optimized."""
    def __init__(
        self,
        dim: int,
        num_heads: int,
...
class GPT(nn.Module):
    """GPT model with Block Attention Residuals, optimized for FP8 training."""
    def __init__(
        self,
        vocab_size: int,
        num_layers: int,
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        # Use final partial_block as output
        x = partial_block
...
def main() -> None:
    ...
    # H100 verification and FP8 setup
    device_capability = torch.cuda.get_device_capability()
    if device_capability < (9, 0):
        raise RuntimeError(f"H100 GPU required (compute capability 9.0+), got {device_capability}")

    # Fast math knobs for H100
    ...
    log0(f"FP8 Training: dtype={fp8_dtype}, H100 optimized=True", console=False)
    log0("=" * 100, console=False)
    ...
    log0(f"fp8_optimization dtype={fp8_dtype}")
    log0(
