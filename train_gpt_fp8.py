"""
FP8 Training variant of train_gpt.py
Uses PyTorch native FP8 (float8_e4m3fn) for forward activations and float8_e5m2 for gradients.
"""
...
class FP8Linear(nn.Module):
    """
    FP8 Linear layer with delayed scaling and H100 FP8 matmul support.
    Uses float8_e4m3fn for activations/weights and float8_e5m2 for gradients.
    """
    def __init__(
        self,
        in_features: int,
...
class FP8Embedding(nn.Embedding):
    """FP8-friendly embedding that keeps weights in FP32."""
    def forward(self, input: Tensor) -> Tensor:
        # Cast to BF16 for compute (embeddings don't benefit from FP8 as much)
        return F.embedding(
            input,
...
# -----------------------------
# MUON OPTIMIZER (same as original)
# -----------------------------
...
# -----------------------------
# TOKENIZER-AGNOSTIC EVALUATION (same as original)
# -----------------------------
...
# -----------------------------
# ATTENTION RESIDUALS (same as original)
# -----------------------------
...
# -----------------------------
# TRANSFORMER MODULES (FP8-optimized)
# -----------------------------
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        # Use FP8Linear for training efficiency
        self.c_q = FP8Linear(dim, dim, bias=False)
        self.c_k = FP8Linear(dim, kv_dim, bias=False)
        self.c_v = FP8Linear(dim, kv_dim, bias=False)
...
class MLP(nn.Module):
    def __init__(self, dim: int, mlp_mult: int):
        super().__init__()
        hidden = mlp_mult * dim
        self.fc = FP8Linear(dim, hidden, bias=False)
        self.proj = FP8Linear(hidden, dim, bias=False)
        self.proj._zero_init = True
...
class GPT(nn.Module):
    def __init__(
        ...
        # Use FP8-friendly embedding
        self.tok_emb = FP8Embedding(vocab_size, model_dim)
        ...
        self.lm_head = None if tie_embeddings else FP8Linear(model_dim, vocab_size, bias=False)
...
def main() -> None:
    ...
    log0(f"FP8 training enabled: output_grad={args.fp8_output_grad}")
    ...
    log0(f"FP8 training: enabled (delayed scaling, output_grad={args.fp8_output_grad})")
    log0(
    ...
    while True:
        ...
        for micro_step in range(grad_accum_steps):
            if distributed:
                model.require_backward_grad_sync = micro_step == grad_accum_steps - 1
            x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
            # Use BF16 autocast - FP8Linear handles FP8 internally
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                loss = model(x, y)
